"""
Convert Arpita's portrait photo into a monochrome ASCII-art SVG
that "types" itself in line by line like a terminal.

Outputs: arpita-ascii.svg
"""
...
OUT = sys.argv[2] if len(sys.argv) > 2 else os.path.join(HERE, "..", "arpita-ascii.svg")
...
FRAME = "#30363d"
TITLE_TEXT = "#7d8590"
INK = "#c9d1d9"      # light-gray ascii color
CURSOR = "#c9d1d9"

ROW_DUR = 0.11
STAGGER = 0.11       # cursor sweeping down

# 1. sample image into grid
if not os.path.exists(SRC):
    # fallback to source-photo.png directly if prepped image not created yet
...
parts.append(f'<text x="{CANVAS_W/2}" y="{TITLEBAR_H/2 + 4}" fill="{TITLE_TEXT}" font-size="12" '
             f'text-anchor="middle">arpita@github: ~$ ./portrait.sh</text>')
...
parts.append(f'<text x="{PAD}" y="{status_y:.1f}" fill="{TITLE_TEXT}" font-size="13">'
             f'arpita@github:~$ whoami <tspan fill="{INK}">Arpita Nathwani</tspan></text>')
...
# Also write avi-ascii.svg as alias so any links remain valid
alias_out = os.path.join(HERE, "..", "avi-ascii.svg")
