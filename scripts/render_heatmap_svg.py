#!/usr/bin/env python3
"""
Render data/contributions.json as a GitHub contribution heatmap SVG:
53-week x 7-day calendar of rounded colored boxes with diagonal reveal keyframe animation,
Less->More legend, and statistics footer.
Width: 860px (aligned with 370px portrait + 490px info card).
...
def render(data):
    ...
    canvas_w = 860 # Exact 860px container width
    stats_h = 88
...
""".strip()
    ...
    parts.append(f'<text x="{canvas_w/2}" y="{TITLEBAR_H/2 + 4}" fill="{MUTED}" font-size="12" '
                 f'text-anchor="middle">arpita@github: ~/contributions --graph</text>')

    grid_top = TITLEBAR_H + TOP_LABEL_H
    grid_left = PAD + LEFT_LABEL_W + 10
...
if __name__ == "__main__":
    if not os.path.exists(IN_PATH):
        # generate data if contributions.json not present yet
